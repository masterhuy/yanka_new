jQuery(function ($) {
  ...
  $(document).on('click', '.template-import', function(event){
      ...
      $.ajax({
          type: 'POST',
          url: PagebuilderConfig.editor_link + '&action=ImportTemplate&ajax=1',
          data: {
  ...
  */
  ...
  checkEmpty = function(iframeDoc) {
      ...
      $(iframeDoc).find('.layout-column').each(function(event) {
  ...
  dragDropInit = function(iframeDoc) {
      ...
      $(iframeDoc).on('dragenter', '.layout-column', function(event)
      {
          ...
          if($(dragged).hasClass('addon-cat-addons') && ($(dragged).find('.disabled').length == 0)) {
