...
class gdz_themesetting extends Module
{
    ...
    public function install()
    {
        ...
        if (parent::install() && $this->registerHook('displayBackOfficeHeader')) {
            ...
            /*if(((int)Tab::getIdFromClassName('PRESTAWORK') > 0) && ((int)Tab::getIdFromClassName('AdminGdzThemeSettingLicense') <= 0)) {
                $tab = new Tab();
                $tab->active = 1;
                $tab->class_name = "AdminGdzThemeSettingLicense";
                $tab->position = 4;
                $tab->icon = 'settings';
                $tab->name = array();
                foreach (Language::getLanguages(true) as $lang) {
    ...
    public function uninstall()
    {
        ...
        if (parent::uninstall()) {
            ...
            $tab = new Tab($id_tab);
            $res &= $tab->delete();
            /*$id_tab = (int)Tab::getIdFromClassName('AdminGdzThemeSettingLicense');
            $tab = new Tab($id_tab);
            $res &= $tab->delete();*/
            return $res;
        }
