						...
						<div class="dummyfile input-group">
							...
							<span class="input-group-btn">
								<button id="{$input.name|escape:'html':'UTF-8'}_{$language.id_lang|escape:'html':'UTF-8'}-selectbutton" type="button" name="submitAddAttachments" class="btn btn-default">
									<i class="icon-folder-open"></i> {l s='Choose a file' mod='gdz_megamenu'}
								</button>
							</span>
						</div>
					</div>
...
{/if}
	...
	<script type="text/javascript">
		...
		var msg_select_one = "{l s='Please select at least one product.' js=1 mod='gdz_megamenu'}";
		var msg_set_quantity = "{l s='Please set a quantity to add a product.' js=1 mod='gdz_megamenu'}";
				...
				$('document').ready( function() {
					$(".check_product_name")
								...
								formatItem: function(data, i, max, value, term) {
									...
									if ($('.ac_results').find('.separation').length == 0)
										$('.ac_results').css('background-color', '#EFEFEF')
											.prepend('<div style="color:#585A69; padding:2px 5px">{l s='Use a product from the list' mod='gdz_megamenu'}<div class="separation"></div></div>');
									return value;
								},
								parse: function(data) {
									var mytab = new Array();
						...
						.result(function(event, data, formatted) {
							...
							jConfirm('{l s='Do you want to use this product?' mod='gdz_megamenu'}&nbsp;<strong>'+data.name+'</strong>', '{l s='Confirmation' mod='gdz_megamenu'}', function(confirm){
								if (confirm == true)
		...
		{/if}
			...
			var no_related_product = '{l s='No related product' mod='gdz_megamenu'}';
...
<script type="text/javascript">
	...
	var missing_product_name = '{l s='Please fill product name input field' js=1 mod='gdz_megamenu'}';
</script>
